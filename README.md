![](banner.png)



# Hi, I'm Simone 👋

Cybersecurity student focused on penetration testing, OSINT, and secure infrastructure analysis.  
I build educational pentest simulations using OWASP, PTES, and CVSS v3.1.

---

## 🔐 Skills

- Penetration Testing  
- OSINT & Reconnaissance  
- Network Analysis  
- TLS/HTTPS Security  
- Email Security (SPF, DKIM, DMARC)  
- CVSS v3.1 Scoring  
- Technical Documentation  
- GitHub Project Structuring  

---

## 📂 Featured Projects

### 🔸 Pentest Simulation – RetailCompany  
Educational penetration testing project (GDPR-safe)  
➡️ https://github.com/simonesimoncini97ss-web/pentest-simulation-retailcompany

---

## 🎯 Goals

- Become a cybersecurity analyst specialized in penetration testing  
- Improve skills in OSINT, network security, and secure infrastructure analysis  
- Build a complete cybersecurity portfolio with multiple simulations  

---

## 🌐 Social

- **LinkedIn:** https://www.linkedin.com/in/simone-simoncini-145894241/  
- **Email:** simonesimoncini97.ss@gmail.com  

---

## 🧭 About Me

I work on educational penetration testing simulations using industry methodologies such as OWASP WSTG, PTES, and CVSS v3.1.  
I focus on OSINT, network analysis, TLS security, email security posture, and vulnerability assessment.  
My goal is to become a cybersecurity analyst specialized in penetration testing and secure infrastructure analysis.

---

## 🛠 Future Portfolio Projects

- TLS Security Analysis Lab  
- Email Security Lab (SPF, DKIM, DMARC)  
- OSINT Investigation Lab  
- Pentest Simulation – Healthcare  
- Pentest Simulation – Banking  
- Cybersecurity Portfolio (full index of all projects)

---

## ⭐ Philosophy

Security is not just about finding vulnerabilities —  
it's about understanding systems, documenting clearly, and improving them ethically.
